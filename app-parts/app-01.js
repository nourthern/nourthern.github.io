...
function bindSettings(rebind=false){
  ...
  ids.forEach(id=>{const el=$(id);el.value=state.settings[id]??'';if(!rebind&&!el.dataset.bound){const save=()=>{state.settings[id]=el.value;if(id==='baseSite')state.settings.baseSiteConfirmed=!!el.value;updateWorkflowHints();saveState();};el.addEventListener('input',save);el.addEventListener('change',save);el.dataset.bound='1';}});
