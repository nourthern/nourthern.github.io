...
function updateAdditionalExpenseUi(){
  ...
  const otherType=otherTypeSelect?.value||'',otherChosen=showOther&&!!otherType;if($('otherExpenseCostRequiredMark'))$('otherExpenseCostRequiredMark').hidden=!otherChosen;if($('extraCostSharingNote'))$('extraCostSharingNote').hidden=!(hasExpense&&otherChosen);
...
function bindMileageRateLock(rebind=false){const field=$('mileageRate'),dialog=$('mileageRateDialog');if(!field||!dialog)return;field.readOnly=!state.settings.mileageRateUnlocked;field.setAttribute('aria-readonly',String(field.readOnly));if(rebind||field.dataset.rateLockBound)return;const requestEdit=event=>{if(!field.readOnly)return;event.preventDefault();dialog.showModal();};field.addEventListener('click',requestEdit);field.addEventListener('keydown',event=>{if((event.key==='Enter'||event.key===' ')&&field.readOnly)requestEdit(event);});const close=()=>dialog.close();$('cancelMileageRateEdit')?.addEventListener('click',close);$('keepMileageRateLocked')?.addEventListener('click',close);$('confirmMileageRateEdit')?.addEventListener('click',()=>{state.settings.mileageRateUnlocked=true;field.readOnly=false;field.setAttribute('aria-readonly','false');saveState();dialog.close();field.focus();field.select();});field.dataset.rateLockBound='1';}
function bindSettings(rebind=false){
  ...
  bindMoneyField('commuteCost','commuteCost',rebind);bindMoneyField('otherExpenseCost','otherExpenseCost',rebind);bindMileageRateLock(rebind);
