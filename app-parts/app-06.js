...
renderShifts=function(){let migrated=false;selectedShiftItems().forEach(item=>{if(item.category==='work'&&!shiftStatusWasEdited(item)&&item.status!=='Claim'){item.status='Claim';item.reviewed=true;migrated=true;}});if(migrated)saveState();renderShiftsAccessible();
