...
$('prefillExampleData')?.addEventListener('click',()=>{
  ...
  const now=londonParts(new Date()),key=`${now.year}-${String(now.month).padStart(2,'0')}`,events=[];for(let i=0;i<22;i++){const day=i+1,date=`${key}-${String(day).padStart(2,'0')}`,isNight=i%5===0,start=londonDateTime(date,isNight?'20:00':'08:00'),end=isNight?londonDateTime(offsetLondonDateKey(date,1),'08:00'):londonDateTime(date,'17:00'),uid=`example-${key}-${String(day).padStart(2,'0')}`;events.push({id:uid,uid,summary:isNight?'Example night shift':'Example day shift',description:'Synthetic testing data',start:start.toISOString(),end:end.toISOString(),status:'Claim',category:'work',source:'Example data'});}
