...
**v6 workflow fix:** repairs the fresh-install JavaScript startup crash, adds clear calendar status feedback, always shows the current month plus four previous claim months, and adds Shifts → Claim Form and Claim Form → Expense Log workflow buttons.
...
1. Enter and save the ICS URL and Worker URL in **Setup**.
2. Open **Shifts** and press **Fetch calendar**, or import an `.ics` file directly.
3. The page now shows a visible success/error status.
4. Select any claim month from the current month or previous four months. Each month shows the number of imported shifts it contains.
5. Press **Continue to Claim Form**. The first selected month is prepared automatically.
6. Review/sign/export the PDF.
7. Press **Continue to Expense Log**. Use **Mark submitted** if you want the claim recorded in the running expense log.

If you already deployed the Cloudflare Worker from the previous package, you do **not** need to redeploy it for this v6 website fix; the Worker code remains compatible.
