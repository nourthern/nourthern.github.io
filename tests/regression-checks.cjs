...
assert.match(workerConfig, /"pattern": "pier\.bynour\.uk"/);
assert.match(workerConfig, /"custom_domain": true/);
assert.match(workerConfig, /"directory": "\.\.\/site-dist"/);
...
const helperStart = app03.indexOf("let shiftView='calendar'");
const helperEnd = app03.indexOf('function updateStudyProgress', helperStart);
const labelStart = app03.indexOf('function generatedShiftLabel');
const labelEnd = app03.indexOf('function groupLinkedEvents', labelStart);
assert.ok(helperStart >= 0 && helperEnd > helperStart && labelStart >= 0 && labelEnd > labelStart, 'Shift classification helpers were not found');
const shiftSandbox = {};
vm.runInNewContext(`${app03.slice(helperStart, helperEnd)}\n${app03.slice(labelStart, labelEnd)}\nresult={generatedShiftLabel,nightOverlapHours,includesNightHours,calendarMonthCells,calendarRoundedRange};`, shiftSandbox);
const { generatedShiftLabel, nightOverlapHours, includesNightHours, calendarMonthCells, calendarRoundedRange } = shiftSandbox.result;
const shift = (start, end, summary = 'Clinical shift') => ({ start, end, summary, description: '' });
assert.equal(generatedShiftLabel(shift('2026-06-01T12:00:00', '2026-06-01T22:00:00')), 'Long afternoon day');
assert.equal(generatedShiftLabel(shift('2026-06-01T08:00:00', '2026-06-01T18:00:00')), 'Long day shift');
assert.equal(generatedShiftLabel(shift('2026-06-01T17:00:00', '2026-06-02T01:00:00')), 'Evening shift');
assert.equal(generatedShiftLabel(shift('2026-06-01T17:00:00', '2026-06-02T02:00:00')), 'Night shift');
assert.equal(generatedShiftLabel(shift('2026-06-01T12:00:00', '2026-06-01T13:00:00')), 'Afternoon shift');
assert.equal(generatedShiftLabel(shift('2026-06-01T11:00:00', '2026-06-01T12:30:00')), 'Morning shift');
assert.equal(generatedShiftLabel(shift('2026-06-01T08:00:00', '2026-06-01T16:00:00')), 'Day shift');
assert.equal(generatedShiftLabel(shift('2026-06-01T16:00:00', '2026-06-02T00:00:00')), 'Afternoon shift');
assert.equal(generatedShiftLabel(shift('2026-06-01T11:00:00', '2026-06-01T21:00:00')), 'Long day shift');
assert.equal(generatedShiftLabel(shift('2026-06-01T12:00:00', '2026-06-01T22:00:00', 'Study day')), 'Self-Development Time');
assert.equal(nightOverlapHours('2026-06-01T17:00:00', '2026-06-02T02:00:00'), 3);
assert.equal(includesNightHours('2026-06-01T20:00:00', '2026-06-02T01:00:00'), false);
assert.equal(includesNightHours('2026-06-01T20:00:00', '2026-06-02T05:00:00'), true);
assert.equal(calendarMonthCells('2026-06').indexOf(1), 0);
assert.equal(calendarMonthCells('2026-08').indexOf(1), 5);
assert.equal(calendarRoundedRange(shift('2026-06-01T07:45:00', '2026-06-01T17:15:00')), '08–17');
...
assert.match(app, /const APP_SHELL_VERSION='33'/);
...
assert.match(html, /<h2>Check shifts<\/h2>/);
assert.match(html, /id="calendarViewButton"[^>]+aria-pressed="true"/);
assert.match(html, /id="listViewButton"[^>]+aria-pressed="false"/);
assert.match(html, /id="shiftCalendar"/);
assert.match(html, /id="clearShiftDateFilter"/);
assert.match(html, /id="shiftDayDialog"[^>]+aria-labelledby="shiftDayDialogTitle"/);
assert.match(html, /Review them if needed, or continue to the Claim Form/);
assert.match(html, /id="localIcsPanel" class="local-ics"/);
assert.match(html, /Choose ICS file<input id="icsFile"[^>]+hidden/);
assert.match(app02, /Automatic calendar refresh failed\. Check the link and try again, or import an \.ics file directly below\./);
assert.match(app02, /\$\('localIcsPanel'\)\.hidden=false/);
assert.match(app04, /if\(panel\)panel\.hidden=false/);
assert.match(app03, /data-date=/);
assert.match(app03, /\.shift-time/);
assert.match(app03, /Study day checked/);
assert.match(app03, /Night shift/);
assert.match(app03, /hasLeave\?'<span>Planned leave<\/span>'/);
assert.match(app03, /aria-haspopup="dialog"/);
assert.match(app03, /function openShiftDay\(dateKey\)/);
assert.match(app03, /item\.category==='study'&&!item\.reviewed/);
assert.match(app03, /studyClaim\?'✓':'!'/);
assert.match(app03, /event\.target===\$\('shiftDayDialog'\)/);
assert.match(app03, /<fieldset class="shift-status-options">/);
assert.match(app03, /function shiftRowMarkup\(item,\{popup=false\}=\{\}\)/);
assert.match(app03, /items\.map\(item=>shiftRowMarkup\(item,\{popup:true\}\)\)/);
assert.match(app03, /class="shift-time" data-field="start" type="time"/);
assert.match(app03, /class="shift-status" type="radio"/);
assert.match(app03, /value="Claim"/);
assert.match(app03, /value="Do not claim"/);
assert.doesNotMatch(app03, /<select class="shift-status"/);
assert.doesNotMatch(app03, /Long evening Shift|Long shift/);
const shiftsContinueHandler = app01.match(/\$\('shiftsContinue'\)\.addEventListener\('click',[^\n]+/)?.[0] || '';
assert.ok(shiftsContinueHandler, 'Continue-to-claim handler was not found');
assert.doesNotMatch(shiftsContinueHandler, /reviewed|study/i);
assert.match(styles, /\.shift-calendar\{display:grid;grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/);
assert.match(styles, /\.shift-month-card:only-child\{grid-column:1\/-1;width:min\(100%,720px\);justify-self:center\}/);
assert.match(styles, /\.shift-view-toggle::before\{[^}]+background:var\(--cool-dusk\)/);
assert.match(styles, /\.shift-view-toggle:has\(#listViewButton\[aria-pressed="true"\]\)::before\{transform:translateX\(100%\)\}/);
assert.match(styles, /\.shift-status-options\{display:grid;grid-template-columns:1fr 1fr/);
assert.match(styles, /\.shift-status-option input:checked\+span::before\{content:"✓"/);
assert.match(styles, /\.shift-status-option input:checked\+span\{border-color:transparent;background:transparent;color:var\(--cool-dusk\)\}/);
assert.match(styles, /\.shift-day-dialog\{width:min\(1000px,calc\(100% - 30px\)\)/);
assert.doesNotMatch(styles, /\.panel:target|body:has\(\.panel:target\)/);
assert.match(styles, /@media\(max-width:820px\)\{\.shift-calendar\{grid-template-columns:minmax\(0,1fr\)\}/);
...
assert.match(sw, /pier-travel-expense-manager-v46/);
...
assert.match(app05, /const APP_VERSION='33'/);
...
assert.match(workerConfig, /"run_worker_first": \["\/ics", "\/dashboard\*", "\/api\/\*"\]/);
