...
assert.match(html, /id="openDashboard"[^>]+Open private telemetry dashboard[^>]+hidden/);
assert.match(styles, /\.beta-dashboard-icon::after\{content:'BETA'/);
assert.match(app05, /dashboard\.hidden=channel!=='beta'/);
assert.match(app05, /openDashboard.*location\.assign\(BAKED_WORKER_URL.*\/dashboard/);
