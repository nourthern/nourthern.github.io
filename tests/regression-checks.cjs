...
assert.match(html, /<h1>PI£R<\/h1>/);
assert.match(html, /Painless &amp; Intelligent Expenses Reporting tool/);
assert.match(html, /styles\.css\?v=18/);
assert.match(html, /app\.js\?v=18/);
assert.match(app, /const APP_SHELL_VERSION='18'/);
...
assert.match(privacyDialog, /What We NEVER Collect/);
assert.match(html, /id="helpDialog"/);
assert.match(html, /use the save icon to create a backup of tabs 1 and 4/);
...
assert.match(styles, /--teal:#2A9D8F/);
assert.match(styles, /--seafoam:#6BCDBD/);
assert.match(styles, /--coral:#FF8A65/);
assert.match(styles, /--sunshine:#FFC857/);
assert.match(styles, /--sand:#F7F2E5/);
assert.match(styles, /--coastal-blue:#B7D6E8/);
assert.match(styles, /Preserve the official claim document preview and print appearance/);
assert.match(app05, /const APP_VERSION='18'/);
assert.match(sw, /travel-claims-manager-v23/);
