...
const dashboard = fs.readFileSync(path.join(root, 'dashboard-assets', 'dashboard.js'), 'utf8');
const dashboardHtml = fs.readFileSync(path.join(root, 'dashboard-assets', 'index.html'), 'utf8');
...
assert.match(app, /const APP_SHELL_VERSION='61'/);
...
assert.match(html, /Only change this if advised of a new rate by Payroll\./);
assert.match(html, /id="mileageRateDialog"/);
assert.match(html, /Choose up to four months to start your claim\./);
assert.match(html, /Click on any shift to view, edit or delete\./);
assert.doesNotMatch(html, /Regular additional expense/);
assert.doesNotMatch(html, /Passenger names and passenger miles are both required when either is entered\./);
assert.doesNotMatch(html, /Average commute time to work/);
assert.match(html, /id="homeAddress"[\s\S]+id="baseSite"[\s\S]+id="vehicleReg"/);
assert.match(html, /The signing date is added when you save each claim form\.[\s\S]+id="setupSignaturePreview"[\s\S]+id="setupSignature"/);
...
assert.match(app06, /initializePierAccessibility\(\)/);
assert.doesNotMatch(app06, /Display &amp; accessibility/);
...
assert.match(app06, /shift-weekday/);
assert.match(app06, /marker\.textContent='×'/);
...
assert.match(app04, /file\.size>5\*1024\*1024/);
assert.match(app04, /Save signature to use it/);
...
assert.match(html, /<h1>Check shifts<\/h1>/);
assert.match(html, /<h1>Review and save<\/h1>/);
assert.match(html, /<h1>Expense log<\/h1>/);
assert.match(html, /<caption>Your locally saved claim records<\/caption>/);
assert.match(html, /id="restoreInput" class="visually-hidden-file"/);
assert.match(html, /id="icsFile" class="visually-hidden-file"/);
...
assert.match(styles, /\.brand-block p\{[^}]*font-weight:600/);
...
assert.match(sw, /pier-travel-expense-manager-v61/);
...
assert.match(app05, /const APP_VERSION='61'/);
...
assert.match(dashboardHtml, /data-channel="live"/);
assert.match(dashboardHtml, /data-channel="beta"/);
assert.match(dashboardHtml, /id="bulkDelete"/);
...
assert.match(dashboardHtml, /data-channel="live" aria-pressed="true"/);
assert.ok(dashboardHtml.indexOf('id="telemetryDefinitions"') < dashboardHtml.indexOf('class="environment"'), 'Telemetry definitions must appear before the environment switch.');
assert.doesNotMatch(dashboard, /metric-definition/);
for (const label of ['Returning users','ICS file share','Shifts imported per claim','Average time to first PDF','Humber Bridge clicks','Email payroll clicks','Push failures']) assert.match(dashboard, new RegExp(label));
for (const definition of ['Network','Opened','Claim','Log']) assert.match(dashboardHtml, new RegExp(`<dt>${definition}<\\/dt>`));
...
assert.doesNotMatch(worker, /function dashboardHtml/);
...
assert.match(fs.readFileSync(path.join(root,'app-parts/astral.js'),'utf8'), /Saturation/);
...
// Synthetic saved-state financial regression: preserve miles, repair stale money.
const financeContext={};
vm.runInNewContext(app00.slice(app00.indexOf('const APPROVED_MILEAGE_RATE'),app00.indexOf('const STORAGE_KEY'))+`
this.reconcile=normalizeExpenseLog;this.amount=mileageAmount;`,financeContext);
const legacy=[{id:'synthetic-a',miles:100,misc:5,totalMiles:2,total:7},{id:'synthetic-b',miles:12.5,misc:0,totalMiles:0.25,total:0.25}];
const corrected=financeContext.reconcile(legacy);
assert.equal(corrected[0].totalMiles,30);
assert.equal(corrected[0].total,35);
assert.equal(corrected[1].total,3.75);
assert.equal(corrected.reduce((sum,item)=>sum+item.miles,0),112.5);
assert.equal(legacy[0].total,7,'Do not mutate the imported source object');
assert.equal(financeContext.amount(0),0);
assert.equal(financeContext.amount(0.1),0.03);
assert.equal(financeContext.reconcile([{miles:0,misc:2}])[0].total,2);
assert.deepEqual(JSON.parse(JSON.stringify(financeContext.reconcile(corrected))),JSON.parse(JSON.stringify(corrected)));
assert.match(html, /id="logMilesTotal"/);
assert.match(app04, /fmtNum\(x.miles\)\} miles/);
assert.match(app00, /mileageRateUnlocked/);
assert.match(app01, /bindMileageRateLock/);
assert.match(styles, /\.undo-region\{position:fixed;left:18px/);
