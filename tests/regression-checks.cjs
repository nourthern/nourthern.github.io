...
assert.match(app, /const APP_SHELL_VERSION='53'/);
...
assert.match(sw, /pier-travel-expense-manager-v53/);
...
assert.match(app05, /const APP_VERSION='53'/);
...
assert.equal((html.match(/<h1>/g)||[]).length,4);
...
assert.match(html, /Date form created/);
assert.match(html, /Open payroll email draft/);
assert.doesNotMatch(app05, /setTimeout\(\(\)=>openDialog\('outcomeSurveyDialog'\)/);
...
assert.match(styles, /font-size:1rem;line-height:1\.55/);

assert.match(html, /Created claims are logged locally/);
assert.match(html, /Date form created/);
assert.match(html, /Miscellaneous owed \(£\)/);
assert.match(html, /Total owed \(£\)/);
assert.match(app04, /No created claims yet/);
